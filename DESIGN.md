...
|---|---|---|
| `public-ip` | IP public qua **endpoint do user cấu hình** (danh sách URL, thử lần lượt có fallback, HTTP client pin theo address family). Response chỉ được chấp nhận khi parse ra **đúng một địa chỉ IPv4/IPv6 hợp lệ** (theo `mode: v4/v6`) — không nhận HTML/chuỗi rác | Use case gốc của emday. Tách khỏi `local-ip` từ v0.1.1 (trước đó gộp trong `type: ip`) |
| `local-ip` | IP của NIC theo `interfaces:`, đọc từ kernel — không gọi mạng | Server LAN-only chỉ cần source này |
