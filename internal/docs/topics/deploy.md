...
must hand them over explicitly, via an `EnvironmentFile` readable only by
root/the service user.

`emday init` seeds a filled-in-the-blanks template so you do not have to
recall the variable names: **`<config-dir>/emday.env.example`**, listing every
`EMDAY_*` variable the starter config can reference, the unused ones commented
out. emday **does not read it** — nothing loads it automatically. It is a
template you copy to wherever your unit points:

    # 1. fill in the values
    sudoedit /etc/emday/emday.env.example

    # 2a. unit generated by `emday install` — it already reads this path:
    sudo install -m 0600 /etc/emday/emday.env.example /etc/sysconfig/emday

    # 2b. or your own unit — copy it where EnvironmentFile= points:
    sudo install -m 0600 /etc/emday/emday.env.example /etc/emday/emday.env

    # 3. the env is read at process start, so a restart is required
    sudo systemctl restart emday

The end state either way is a `KEY=VALUE` file, mode 0600:

    # /etc/emday/emday.env   (rendered from your secret store)
    EMDAY_TG_TOKEN=123456:ABC-...
    EMDAY_LARK_SECRET=xxxxxxxx
    EMDAY_SLACK_URL=https://hooks.slack.com/services/T/B/X

No `export`, no shell expansion, no trailing comments — systemd parses the
file itself, not through a shell. Reference it from your own unit with
`EnvironmentFile=/etc/emday/emday.env`, or — if you used `emday install` —
from `/etc/sysconfig/emday` (the generated unit already reads that path, and
its `-` prefix means a **missing file is silently ignored**), or a drop-in:

    systemctl edit emday        # add [Service] EnvironmentFile=/etc/emday/emday.env

On a config-managed host that never runs `init`, ignore the template and
render the `EnvironmentFile` from your secret store directly.
